...
def drawCMS(x,y,size=0.05):
    ...
    latex.DrawLatex(x+size*2,y-0.005,"Preliminary")

    return

def drawDate(x,y,font=42):
    latex = ROOT.TLatex() 
    latex.SetNDC(1)
...
def cosmetic(graph):
    name = graph.GetName()
    if "zh"  in name:graph.SetLineColor(ROOT.kRed-4)#Blue-4) 
    if "csc" in name:graph.SetLineColor(ROOT.kBlue-4)
    if "dj"  in name:graph.SetLineColor(ROOT.kTeal+2)#Blue-4) 
...
def plotHiggsLimits(decay="bb",masses=[15,40,55], samples=["zh","dj","csc"]):
    ...
    mgraph.Draw("AL")
    mgraph.SetTitle(";c#tau_{s} [mm];95% CL upper limit on BR(h#rightarrowss)")
    ...
    # lines
    minx,maxx,miny,maxy=5e-1,1.5e7,3e-4,5e3
